...
The main objective is to examine the relationship between litter size and mother's age.
<!-- why single out littersize? -->
We compare data on litter size in wild populations with data from foxes kept in captivity as part of a conservation program. For foxes in captivity, data on pup survival until 8-10 weeks of age. Based on these data the relationship between pup survival and mother's age is examined.
<!-- should we focus on comparing wild and captive? - see review comments -->
...
<!-- dette bør kanskje komme i heading -->
...
For *captive foxes,* litter size data include zeros. For comparison with *wild foxes*, we have fitted a zero-truncated model 
<!-- also for these data.  -->
for both data sets.
<!--  NB! text replacement above -->
This implies that we model the variation in litter size conditional on producing at least one pup that survived until marking.
...
<!-- Her er jeg litt forvirra, for AIC ER jo (LITT) lavere når rodent index er med... Både over og under sies det at AIC blir høyere med rodent_index.  -->
...
The plot does not display any clear relationship between variation in the rodent index and litter size in captivity. Furthermore, including the rodent index as a predictor in the model does not improve the likelihood of the model as assessed by the AIC values. 
<!-- Dette stemmer jo ikke... Liten forskjell, men modellen med 'rodent index' har lavest AIC -->
We therefore exclude rodent index from the final model for litter size in captivity.
...
The expected number of surviving offspring in captivity ($\widehat \Lambda_a$), depending on mother age ($a$), can be computed by multiplying the predicted breeding probability ($\widehat B_a$), litter size ($\widehat L_a$), and offspring survival ($S_a$) for each mother age.

$$
\widehat \Lambda_a = \widehat B_a \widehat L_a \widehat S_a
$$
...
Since $\log(\widehat \Lambda_a) = \log(\widehat B_a) + \log(\widehat L_a) + \log(\widehat S_a)$ and and the three components are estimated independently, we get

$$
\text{Var} \left( \log(\widehat \Lambda_a) \right) =
...
V_log_Lambda <- V_log_B + V_log_L + V_log_S

# Computing approx. 95% CI (central limit theorem) and making a data frame
SE_log_Lambda <- sqrt(V_log_Lambda)
Expected_no_survivors <- data.frame(
  mother_age = 1:9,
  Lambda = Lambda,
  lwr_Lambda = exp(log(Lambda) - 2*SE_log_Lambda),
  upr_Lambda = exp(log(Lambda) + 2*SE_log_Lambda)
)
