...
This document is part of the supplementary materials for the manuscript "**Reproductive senescence in cyclic environments: effects of maternal age on reproductive performance in the Arctic fox**". To run the R code in this document, it is necessary to first download these three datasets: 
...
```{r}
captive <- read.table('data/litter_size_captive_breeding_station.txt', header = TRUE)
wild <- read.table('data/litter_size_wild_populations.txt', header = TRUE)
survival_data <- read.table("data/survival_captive_breeding_station.txt", header = TRUE)
```
...
<br/> <br/> To examine the potential effect of rodents on probability of breeding in captivity, we fitted a second model including that also include 'rodent index' as a fixed effect using a B-spline function to account for non-linear responses:
...
DEBUG! New version of the equation above from Torbjørn:
