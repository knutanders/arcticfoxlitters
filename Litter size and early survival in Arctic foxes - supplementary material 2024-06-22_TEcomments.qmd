...
The main objective is to examine the relationship between litter size and mother's age in the Arctic fox. We compare data on litter size in wild populations with data from a captive breeding program for conservation.
<!-- otherwise it can read as the comparison being a part of a concervation program --> 
For foxes in captivity, there are also data on pup survival until 8-10 weeks age and the relationship between pup survival and mother's age is examined.
...
This document is part of the supplementary materials for the manuscript "......". To run the R code in this document, it is necessary to first download these three datasets: (1) litter_size_captive_breeding_station.rds (2) litter_size_wild_populations.rds (3) survival_captive_breeding_station.rds
...
*Figure S2. Predicted median litter size as a function of female age in wild Arctic foxes.The predicted median is displayed as blue dots and observations as smaller, grey dots. Whiskers indicate 95% confidence intervals of the predicted medians.*

---
KOMMENTAR: Med en log-link og normalfordelte random effects er mean lik $\exp(\lambda_{i,k} + 0.5\sigma_\delta + 0.5\sigma_\epsilon)$. Medianen blir $\exp(\lambda_{i,k})$ som er det som er plottet.
---
...
*Figure S3. Relationship between rodent index and predicted median litter size in wild Arctic foxes. The effect of rodent index on litter size is described by a B-spline function.*
...
<br/> Predictions for foxes in captivity based on model c1:
...
*Figure S5. Predicted median litter size as a function of mother age in captive Arctic foxes.The predicted median is displayed as red dots and observations as smaller, grey dots. Whiskers indicate 95% confidence intervals of the predicted medians.*
...
The data from foxes in captivity include both females breeding and not breeding. This data is not available for wild foxes. For captive foxes we can therefore model the probability of producing a litter (Bernoulli model). The binary outcome ‘breeding’ (1) or ‘no breeding’ (0) of individual foxes in each year is used as the response variable and analysed using a GLMM with a binomial distribution. The model includes the fixed effects 'age of the mother' using a second order polynomial function, and 'rodent index'
<!-- KOMMENTAR: Rodent index er ikke med i første modell beskrvet under, så skriv anderledes -->
using a B-spline function to account for non-linear responses. The 'mother ID' and 'year' were included as random effects to account for correlation in the data.
...
Fitting an alternative model including the variable rodent_index as a predictor:
